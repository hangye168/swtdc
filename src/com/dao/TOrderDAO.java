...
public class TOrderDAO extends HibernateDaoSupport
{
	...
	public List<TOrder> report(int id){
		List<TOrder> list = new ArrayList<TOrder>();
		String sql = "select DATE_FORMAT(order_date,'%Y-%m') orderdate,sum(sjfk) sjfk from  t_order where shid="+id+" and order_zhuangtai='ÒÑ¸¶¿î' group by DATE_FORMAT(order_date,   '%Y-%m')";
		...
		if(olist!=null && olist.size()>0){
			for (Object[] objects : olist) {
				...
				order.setOrderDate(objects[0]+"");
				order.setSjfk(Integer.parseInt(objects[1]+""));
				list.add(order);
			}
		}
		return list;
