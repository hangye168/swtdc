...
<html>
  ...
  <form action="<%=path%>/pinglunAdd.action" name="form1" method="post">
     <table cellspacing="1" cellpadding="0" width="100%" border="0" bgcolor="#F7F3F7">
            <input type="hidden"  name="gsize" value="${requestScope.gsize}"/>
             
		  <c:forEach items="${requestScope.orderItemList}" var="orderItem" varStatus="status">
		  <tr class="text"   bgcolor="#FFFFFF">
		 
			  <td>菜品名称:${orderItem.goods.goodsName}</td>
			   <input type="hidden"  name="gid${status.index}" value="${orderItem.goods.goodsId}"/>
		  </tr>
		  <tr class="text"   bgcolor="#FFFFFF">
		   
			  <td>评论内容<textarea name="pln" name="plnr${status.index}" rows="8" cols="80">默认好评</textarea> </td>
  	 ...
  	 <table cellspacing="1" cellpadding="0" width="100%" border="0" bgcolor="#F7F3F7">
		  ...
		  <tr class="text" align="center"  bgcolor="#FFFFFF">
			  ...
			  <td>  <input value="提交" type="button" onclick="check()" /></td>
